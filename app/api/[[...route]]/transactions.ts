...
const app = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      if (!id) {
        return c.json({ error: "Missing param: id" }, 400);
      }
  ...
  .patch(
    ...
    async (c) => {
      ...
      if (!id) {
        return c.json({ message: "Missing param: id" }, 400);
      }
  ...
  .delete(
    ...
    async (c) => {
      ...
      // Array of ids generated out of out query, for single delete should be an array of lenght 1.
      const transactionsToDelete = db.$with("transactions_to_delete").as(
        db
          .select({ id: transactions.id })
          .from(transactions)
